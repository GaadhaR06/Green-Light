...
def testfnr():
    ...
    if request.method == 'GET':
        res =[]
        finalscore = 99
        # health = {"depression" :-0.6 , "anxiety" :, "PTSD" :5, 'd':5, 'a':7, 'p':3}
        # health["depression"] = round((health["depression"]/(health['d']+1)))
        
        # health["anxiety"] = round((health["anxiety"]/(health['a']+1)))
       
        # health["PTSD"] = round((health["PTSD"]/(health['p']+1)))
        ...
        for ind in health.values():
            if( 7 <= ind):
               keys = [k for k, v in health.items() if v == ind]
